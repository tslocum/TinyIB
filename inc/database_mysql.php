...
try {
  if (TINYIB_DBHOST == "localhost") {
    ...
    $dsn = "mysql:host=" . TINYIB_DBHOST . ";port=" . TINYIB_DBPORT .
           ";dbname=" . TINYIB_DBNAME;
  }
...
function postByID($id) {
  $result = sqlquery("SELECT * FROM " . TINYIB_DBPOSTS .
    " WHERE id = ?", array($id,)
  );
  if ($result) {
    return $result->fetch();
...
function threadExistsByID($id) {
  $result = sqlquery("SELECT COUNT(*) FROM " . TINYIB_DBPOSTS .
    " WHERE id = ? AND parent = 0", array($id,)
  );
...
function bumpThreadByID($id) {
  $now = time();
  sqlquery("UPDATE " . TINYIB_DBPOSTS .
    " SET bumped = ? WHERE id = ?", array($now, $id)
  );
}
...
function allThreads() {
  $threads = array();
  $results = sqlquery("SELECT * FROM " . TINYIB_DBPOSTS .
    " WHERE parent = 0 ORDER BY bumped DESC"
  );
...
function postsInThreadByID($id) {
  $posts = array();
  $results = sqlquery("SELECT * FROM " . TINYIB_DBPOSTS .
    " WHERE id = ? OR parent = ? ORDER BY id ASC", array($id,$id)
  );
...
function postsByHex($hex) {
  $posts = array();
  $results = sqlquery("SELECT * FROM " . TINYIB_DBPOSTS .
    " WHERE file_hex = ? LIMIT 1", array($hex,)
  );
  while($row = $results->fetch(PDO::FETCH_ASSOC)) {
    $posts[] = $row;
  }
...
function deletePostByID($id) {
  ...
  foreach ($posts as $post) {
    if ($post['id'] != $id) {
      ...
      sqlquery("DELETE FROM " . TINYIB_DBPOSTS .
        " WHERE id = ?", array($id,)
      );
    }
    else {
...
function trimThreads() {
  ...
  if ($limit > 0) {
    $results = sqlquery("SELECT id FROM " . TINYIB_DBPOSTS .
      " WHERE parent = 0 ORDER BY bumped LIMIT 100 OFFSET " . $limit
    );
...
function lastPostByIP() {
  $result = sqlquery("SELECT * FROM " . TINYIB_DBPOSTS .
    " WHERE ip = ? ORDER BY id DESC LIMIT 1", array($_SERVER['REMOTE_ADDR'],)
  );
...
# Ban Functions
function banByID($id) {
  $result = sqlquery("SELECT * FROM " . TINYIB_DBBANS .
    " WHERE id = ?", array($id,)
  );
  return $result->fetch(PDO::FETCH_ASSOC);
}
...
function allBans() {
  $bans = array();
  $results = sqlquery("SELECT * FROM " . TINYIB_DBBANS .
...
function clearExpiredBans() {
  $now = time();
  sqlquery("DELETE FROM " . TINYIB_DBBANS .
    " WHERE expire > 0 AND expire <= ?", array($now,)
  );
}
